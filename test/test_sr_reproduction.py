"""``pii.VMC(pii=False)`` must reproduce NetKet's ``VMC_SR`` bit-for-bit."""
...
def test_reproduces_vmc_sr(use_ntk, on_the_fly):
    ...
    got = _params_after(
        pii.VMC, H, hi, pii=False, use_ntk=use_ntk, on_the_fly=on_the_fly, mode="real"
    )
...
def test_sr_converges_with_small_lr():
    _, hi, H, E0 = tfim()
    vs = make_mcstate(hi, seed=0)
    d = pii.VMC(H, optax.sgd(0.05), variational_state=vs, diag_shift=0.01, mode="real")
