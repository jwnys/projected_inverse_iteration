"""``nkpii.driver.VMC_PII(pii=False)`` must reproduce NetKet's ``VMC_SR`` bit-for-bit."""
...
def test_reproduces_vmc_sr(use_ntk, on_the_fly):
    ...
    got = _params_after(
        nkpii.driver.VMC_PII, H, hi, pii=False, use_ntk=use_ntk, on_the_fly=on_the_fly, mode="real"
    )
...
def test_sr_converges_with_small_lr():
    _, hi, H, E0 = tfim()
    vs = make_mcstate(hi, seed=0)
    d = nkpii.driver.VMC_PII(H, optax.sgd(0.05), variational_state=vs, diag_shift=0.01, mode="real")
