"""Tests for the PII PCT operators (:mod:`nkpii.optimizer.pct`).
...
- the PCTJacobian update equals the **existing dense-PII** implementation;
...
def _model(name, hi):
    return {
        ...
        "pii_RBMRealParams": nkpii.models.RBMRealParams(alpha=1),
        "pii_LogStateVector": nkpii.models.LogStateVectorRealParams(hi),
    }[name]
...
@pytest.mark.parametrize("Qctor", [PCTJacobianDense, PCTJacobianPyTree], ids=["dense", "pytree"])
...
def test_q_jacobian_dense_pytree_agree(name, mode):
    """The dense and PyTree PCT operators produce the same dense ``Q``."""
    _, hi, H, E0 = tfim()
    vs = nk.vqs.FullSumState(hi, _model(name, hi), seed=0)
    Qd = np.asarray(PCTJacobianDense(vs, H, tau=1.2 * E0, diag_shift=1e-3, mode=mode).to_dense())
    Qp = np.asarray(PCTJacobianPyTree(vs, H, tau=1.2 * E0, diag_shift=1e-3, mode=mode).to_dense())
...
def test_q_jacobian_matches_existing_dense_pii(name, mode):
    """``PCTJacobian.solve`` gives the same update as the existing dense-PII path."""
    from nkpii.ngd.common import get_samples_and_pdf, _prepare_weights, _prepare_input
    ...
    # PCTJacobian update with rhs = ½∇E (= ½ · the energy gradient)
    Q = PCTJacobianDense(vs, H, tau=tau, diag_shift=lam, mode=mode)
...
def test_logstatevector_holo_matches_realparams():
    ...
    assert converge(nkpii.models.LogStateVectorRealParams(hi), "complex") < 1e-6
...
def test_complex_params_vs_realparams_same_update(kind):
    ...
    if kind == "logstatevector":
        ...
        real = dpsi(nkpii.models.LogStateVectorRealParams(hi),
        ...
        real = dpsi(nkpii.models.RBMRealParams(alpha=1),
