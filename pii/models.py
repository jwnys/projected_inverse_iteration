...
Use it with ``mode='complex'``: the Jacobian splits the (complex) *output* into
real/imaginary parts, but the parameters are not further split, so the update is
a single real vector of length ``P`` (no parameter doubling).
"""
